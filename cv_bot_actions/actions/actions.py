...
class ActionGiveInfo(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        # Retrieve the current category, previous category, and index from slots
        category = tracker.get_slot("category")
        previous_category = tracker.get_slot("previous_category")
        info_index = tracker.get_slot("list_index") or 0

        # Check if the category has changed and reset the index if it has
        if category != previous_category:
            info_index = 0  # Reset index
            # Set the previous_category to current category
        ...
        info_data = {
            ...
            "personality": [
                ...
                "He also likes to do woodwork in his free time."
            ]
        }
        ...
        if info_index >= len(info_list):
            ...
            return [SlotSet("list_index", 0), SlotSet("category", None), SlotSet("previous_category", None)]  # Reset slots
        ...
        # Update the slots with the new index and category
        return [SlotSet("list_index", info_index), SlotSet("previous_category", category)]
